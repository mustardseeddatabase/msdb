...
class SkuListItemsController < ApplicationController
  ...
  def create
    ...
    item.update_attribute(:canonical, true)
    redirect_to edit_sku_list_path
  end

  # remove an item from the list just by setting its canonical attribute to false
  def destroy
    item = Item.find(params[:id])
    respond_to do |format|
      format.js do # when updating an item via ajax
        item.update_attribute(:canonical, false)
