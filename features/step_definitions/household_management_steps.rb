...
Given /^there is a household in the database$/ do
  ...
  FactoryGirl.create(:household, :perm_address_id => perm_address.id)
end

Given /^there are households with zipcodes "([^"]*)" and "([^"]*)" in the database$/ do |zip1, zip2|
  perm_address1 = FactoryGirl.create(:perm_address, :zip => zip1)
  temp_address1 = FactoryGirl.create(:temp_address, :zip => '')
  perm_address2 = FactoryGirl.create(:perm_address, :zip => '')
  temp_address2 = FactoryGirl.create(:temp_address, :zip => zip2)
  hh1 = FactoryGirl.create(:household_with_docs)
  hh1.perm_address = perm_address1
  hh1.temp_address = temp_address1
  hh2 = FactoryGirl.create(:household_with_docs)
...
Given /^there are households with cities "([^"]*)" and "([^"]*)" in the database$/ do |city1, city2|
  h1 = FactoryGirl.create(:household_with_docs)
  h1.perm_address.update_attribute(:city, city1)
  h2 = FactoryGirl.create(:household_with_docs)
  h2.perm_address.update_attribute(:city, city2)
end

Given /^there is a household with "([^"]*)" "([^"]*)" in the database$/ do |attribute, value|
  FactoryGirl.create(:household, attribute.to_sym => value)
end

Given /^there are households with street names "([^"]*)" and "([^"]*)" in the database$/ do |street_name1, street_name2|
  perm_address1 = FactoryGirl.create(:perm_address, :address => street_name1)
  temp_address1 = FactoryGirl.create(:temp_address, :address => '')
  perm_address2 = FactoryGirl.create(:perm_address, :address => '')
  temp_address2 = FactoryGirl.create(:temp_address, :address => street_name2)
  FactoryGirl.create(:household, :perm_address => perm_address1, :temp_address => temp_address1)
  FactoryGirl.create(:household, :perm_address => perm_address2, :temp_address => temp_address2)
end
...
Given /^there is a household with all three qualification documents in the database$/ do
  ...
  res_qualdoc = FactoryGirl.create(:res_qualdoc,
                        :association_id => household.id,
                        :date => 1.month.ago,
                        :warnings => 0,
                        :vi => 1)
  inc_qualdoc = FactoryGirl.create(:inc_qualdoc,
                        :association_id => household.id,
                        :date => 1.month.ago,
                        :warnings => 0,
                        :vi => 1)
  gov_qualdoc = FactoryGirl.create(:gov_qualdoc,
                        :association_id => household.id,
                        :date => 1.month.ago,
                        :warnings => 0,
                        :vi => 1)
  FactoryGirl.create(:client, :household_id => household.id)
...
Given /^there are clients with last names "([^"]*)" and "([^"]*)" in the database$/ do |client_name1, client_name2|
  FactoryGirl.create(:client, :lastName => client_name1)
  FactoryGirl.create(:client, :lastName => client_name2)
end
...
Given /^there is a household with client "([^"]*)" in the database$/ do |name|
  perm_address1 = FactoryGirl.create(:perm_address)
  temp_address1 = FactoryGirl.create(:temp_address)
  household = FactoryGirl.create(:household, :perm_address => perm_address1, :temp_address => temp_address1)
  FactoryGirl.create(:client, :lastName => name, :household_id => household.id)
  household.clients {|clients| [clients.association(:client)]}
end
