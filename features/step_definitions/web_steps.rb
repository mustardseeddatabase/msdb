...
# for Faker methods look in features/step_definitions/faker_methods.rb
When /^(?:|I )fill in the following with Faker values:$/ do |fields|
  fields.rows_hash.each do |name, value|
    step %{I fill in "#{name}" with "#{eval value}"}
  end
end
...
Then /^the "([^"]*)" date field should equal "([^"]*)"$/ do |arg1, arg2|
...
When /^I confirm the popup$/ do
  sleep 1 # there seems to be a timing issue
...
Then /^"([^"]*)" should have value "([^"]*)"$/ do |field, value|
