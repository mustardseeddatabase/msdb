namespace :ccstb do
  desc 'marks items as preferred as defined by custom at CCStB'
  task :mark_preferred => :environment do
    ...
    Item.where("sku in (#{marked_item_skus})").each{|i| i.update_attribute(:preferred, true)}
    Item.where("sku not in (#{marked_item_skus})").each{|i| i.update_attribute(:preferred, false)}
  end
end
